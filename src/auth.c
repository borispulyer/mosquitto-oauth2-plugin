 ...
 *
 * This function is registered with the broker and executed for each incoming
 * connection attempt. It performs validation of the MQTT username/password
 * combination by querying the configured OAuth2 introspection endpoint.
 *
 * @param event      Event type (unused, expected to be MOSQ_EVT_BASIC_AUTH).
 * @param event_data Pointer to struct mosquitto_evt_basic_auth provided by
 *                   Mosquitto.
 * @param userdata   Plugin specific data pointer supplied during registration.
 * @return           MOSQ_ERR_SUCCESS if authentication succeeds or a mosquitto
...
int oauth2plugin_callback_mosquittoBasicAuthentication(
	...
	if (
		...
		&& strstr(_options->username_validation_template, "%%") == NULL
	...
	for (size_t i = 0; i < replacement_map_count; i++) {
		replacement_map[i].needle = oauth2plugin_template_placeholders[i].placeholder;
		cJSON* item = cJSON_GetObjectItemCaseSensitive(cjson, oauth2plugin_template_placeholders[i].oidc_key);
		if (
			cJSON_IsString(item)
		) {
 ...
 * @return                       MOSQ_ERR_SUCCESS on success, MOSQ_ERR_UNKNOWN
 *                               otherwise.
 */
 ...
 * @return Number of bytes processed. Returning a different value will abort the
 *         transfer.
 */
 ...
 *
 * @param introspection_response Parsed JSON object returned from the
 ...
 * @return       MOSQ_ERR_AUTH, MOSQ_ERR_PLUGIN_DEFER or other mosquitto error
 *               codes based on @p error.
 */
